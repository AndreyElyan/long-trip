...
describe('CancelTrip', () => {
  ...
  const getTrips = new GetTripsByUserId(tripsRepository, usersRepository, passengerRepository);

  it('should be able to cancel trips by trip id', async () => {
    ...
    const { tripId } = tripsRepository.trip_response;

    await tripsRepository.cancelTrip(tripId);

    const trips = await getTrips.execute({
