import { InMemoryCarsRepository } from '@test/repositories/in-memory-cars-repository';
import { InMemoryUsersRepository } from '@test/repositories/in-memory-users-repository';
import { GetCarById } from './get-car-by-id';
import { CreateCar } from './create-car';
import { CarNotFoundError } from '../../errors/cars/car-not-found';

describe('GetCarById', () => {
  let carsRepository: InMemoryCarsRepository;
  const usersRepository = new InMemoryUsersRepository();
  let getCarById: GetCarById;
  let createCar: CreateCar;

  beforeEach(() => {
    carsRepository = new InMemoryCarsRepository();
    getCarById = new GetCarById(carsRepository);
    createCar = new CreateCar(carsRepository, usersRepository);
  });

  it('should return a car when the id exists', async () => {
    const car = {
      userId: 'e3c698fc-a755-4083-a8e8-30a8cc82680f',
      model: 'Gol',
      make: 'Volkswagen',
      year: '2010',
      color: 'Preto',
      licensePlate: 'ABC1236',
      imageUrl: '',
      category: 'car',
    };

    const expectedCarResponse = {
      props: {
        id: 1,
        ...car,
      },
    };

    await createCar.execute(car);

    const result = await getCarById.execute(1);

    expect(result).toEqual(expectedCarResponse);
  });

  it('should throw a CarNotFoundError when the id does not exist', async () => {
    try {
      await getCarById.execute(1);
    } catch (error) {
      expect(error).toBeInstanceOf(CarNotFoundError);
      expect(error.message).toBe('Car not found');
    }
  });
});
